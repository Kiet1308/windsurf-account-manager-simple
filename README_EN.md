...
- **Scope**: simple-side account info dialog (3 tabs → 5 tabs); AccountCard first-row buttons (convert login + check Pro trial added); backend Tauri commands active count 38 → 48. Main project synchronously adjusts button position so both projects share the same UI interaction model
